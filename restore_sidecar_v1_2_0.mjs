 ...
 * Version: v1.2.0-RESTORE-SIDECAR-AUTHORITATIVE
 */
import fs from 'fs';
...
console.log("🚀 Starting Programmatic Sidecar Reversion Sentinel v1.2.0...");
...
// 7. Direct-Write tsconfig.json (Zero-Parsing / Zero-Regex / Guaranteed Pure JSON)
console.log("📄 Re-creating tsconfig.json...");
const tsconfigObj = {
  compilerOptions: {
    target: "ES2020",
...
fs.writeFileSync('tsconfig.json', JSON.stringify(tsconfigObj, null, 2));
...
if (fs.existsSync(localPkgPath)) {
    ...
    try {
        pkg = JSON.parse(fs.readFileSync(localPkgPath, 'utf-8'));
    } catch {
        pkg = { dependencies: {} };
    }
