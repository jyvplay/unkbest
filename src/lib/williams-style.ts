...
export const ARCHETYPES = [{ name: "The Strategist", voice: "Mission Planner" }, { name: "The Sentinel", voice: "Risk Watcher" }];

export function generatePersona(name?: string, seed?: number): any { const personaName = name || "The Strategist"; return { name: personaName, seed: seed || 1000, directive: typeof getPersonaDirective === "function" ? getPersonaDirective(personaName) : "" }; }

export function newSessionSeed(): number { return Math.floor(Math.random() * 1_000_000); }

export type WilliamsPersona = string;
