...
export function buildRepairBlock(defects?: any[]): string { return (defects||[]).map((d: any) => `- [${d.severity}] ${d.id}: ${d.detail}`).join("\n"); }

export interface Defect { id: string; severity: string; category: string; detail: string; }

export interface AdversarialReport { defects: Defect[]; verdict: "pass" | "revise"; rawCritique: string; }
