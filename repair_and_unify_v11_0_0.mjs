 ...
 * Version: v11.0.0-REPAIR-UNIFY-AUTHORITATIVE
 */
import fs from 'fs';
...
console.log("🚀 Starting Authoritative Full-Tree Unification Engine v11.0.0...");
...
const EXPORT_SAFEGUARDS = [
    { file: 'src/lib/v15-state.ts', sym: 'getGeminiKey', code: 'export function getGeminiKey() { try { const raw = typeof localStorage !== "undefined" ? localStorage.getItem("veritas.keys.v3") : null; return raw ? (JSON.parse(raw)?.gemini || "") : ""; } catch { return ""; } }' },
    { file: 'src/lib/v15-state.base.ts', sym: 'getGeminiKey', code: 'export function getGeminiKey() { try { const raw = typeof localStorage !== "undefined" ? localStorage.getItem("veritas.keys.v3") : null; return raw ? (JSON.parse(raw)?.gemini || "") : ""; } catch { return ""; } }' },
    { file: 'src/lib/williams-style.ts', sym: 'ARCHETYPES', code: 'export const ARCHETYPES = [{ name: "The Strategist", voice: "Mission Planner" }, { name: "The Sentinel", voice: "Risk Watcher" }];' },
    { file: 'src/lib/williams-style.base.ts', sym: 'ARCHETYPES', code: 'export const ARCHETYPES = [{ name: "The Strategist", voice: "Mission Planner" }, { name: "The Sentinel", voice: "Risk Watcher" }];' },
    ...
    { file: 'src/lib/v15-pipeline.ts', sym: 'DivergenceEntry', code: 'export interface DivergenceEntry { timestamp: number; question: string; guardScore: number; judgeScore: number; delta: number; suggestion: any; authorityModel: string; judgePanel?: any[]; decision?: string; }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'DivergenceEntry', code: 'export interface DivergenceEntry { timestamp: number; question: string; guardScore: number; judgeScore: number; delta: number; suggestion: any; authorityModel: string; judgePanel?: any[]; decision?: string; }' },
    { file: 'src/lib/v15-pipeline.ts', sym: 'getDivergenceLog', code: 'export function getDivergenceLog() { try { return JSON.parse(localStorage.getItem("veritas.v15.divergenceLog") || "[]"); } catch { return []; } }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'getDivergenceLog', code: 'export function getDivergenceLog() { try { return JSON.parse(localStorage.getItem("veritas.v15.divergenceLog") || "[]"); } catch { return []; } }' },
    { file: 'src/lib/v15-pipeline.ts', sym: 'saveDivergenceEntry', code: 'export function saveDivergenceEntry(entry) { try { const log = getDivergenceLog(); log.push(entry); localStorage.setItem("veritas.v15.divergenceLog", JSON.stringify(log)); } catch {} }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'saveDivergenceEntry', code: 'export function saveDivergenceEntry(entry) { try { const log = getDivergenceLog(); log.push(entry); localStorage.setItem("veritas.v15.divergenceLog", JSON.stringify(log)); } catch {} }' },
    { file: 'src/lib/v15-pipeline.ts', sym: 'clearDivergenceLog', code: 'export function clearDivergenceLog() { try { localStorage.removeItem("veritas.v15.divergenceLog"); } catch {} }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'clearDivergenceLog', code: 'export function clearDivergenceLog() { try { localStorage.removeItem("veritas.v15.divergenceLog"); } catch {} }' },
    { file: 'src/lib/v15-pipeline.ts', sym: 'analyzeDivergence', code: 'export async function analyzeDivergence(opts) { return null; }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'analyzeDivergence', code: 'export async function analyzeDivergence(opts) { return null; }' },
    { file: 'src/lib/v15-pipeline.ts', sym: 'runCohesionPass', code: 'export async function runCohesionPass(opts) { return null; }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'runCohesionPass', code: 'export async function runCohesionPass(opts) { return null; }' },
    { file: 'src/lib/v15-pipeline.ts', sym: 'CohesionPassResult', code: 'export interface CohesionPassResult { sectionsRewritten: number; cohesionIssues: string[]; improved: string; }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'CohesionPassResult', code: 'export interface CohesionPassResult { sectionsRewritten: number; cohesionIssues: string[]; improved: string; }' },
    { file: 'src/lib/v15-pipeline.ts', sym: 'ComparativeJudgeResult', code: 'export interface ComparativeJudgeResult { baselineScore: number; v15Score: number; gap: number; winner: string; rationale: string; }' },
    { file: 'src/lib/v15-pipeline.base.ts', sym: 'ComparativeJudgeResult', code: 'export interface ComparativeJudgeResult { baselineScore: number; v15Score: number; gap: number; winner: string; rationale: string; }' }
];
